		...
		<div>
			...
			<form action="productCartegorySelect">
				<select name="column" style="height: 25px;">
					...
					type="hidden" name="p_Cartegory" value="${p_Cartegory }"> <input
					type="submit" value="검색">
			</form>

		</div>
		<p style="text-align: right; margin: auto; padding-right: 15%;">게시물
			수 : ${totalRecord }</p>
				...
				<c:forEach var="pDTO" items="${pDTO }">
					<li class="left-box"><a
						href="productViewPage?p_No=${pDTO.p_No }"> <img
							...
							src="${pageContext.request.contextPath }/resources/storage/${pDTO.p_Img_Path }"></a><br />
						<div class="desc">
							<h3 class="desc-title">${pDTO.p_Name }</h3>
							<br>
							<fmt:formatNumber value="${pDTO.p_Price }" />
							원<br /> 판매자 : ${pDTO.m_Id }<br />
						</div>
				</c:forEach>
