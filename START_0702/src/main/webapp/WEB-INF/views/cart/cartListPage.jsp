			...
			<c:if test="${not empty cDTO }">
				...
				<c:forEach var="cDTO" items="${cDTO }">
					<tr style="height: 150px">
						<td><a href="productViewPage?p_No=${cDTO.p_No }"><img
								alt="${cDTO.p_Img_Path }" width="100px" height="100px"
								src="${pageContext.request.contextPath }/resources/storage/${cDTO.p_Img_Path }"></a></td>
						<td>${cDTO.p_Name }</td>
						<td><fmt:formatNumber value="${cDTO.p_Price }" />원</td>
						<td>${cDTO.m_Id }</td>
						<td><c:if test="${cDTO.p_State eq 1 }">
								판매중
							</c:if> <c:if test="${cDTO.p_State eq 2 }">
								승인대기중
							</c:if> <c:if test="${cDTO.p_State eq -1 }">
								판매완료
							</c:if></td>
						<td>
							<form name="f">
								<input type="hidden" name="c_No" value="${cDTO.c_No }" /> <input
									type="hidden" name="m_Id" value="${mDTO.m_Id }" /> <input
									type="image" alt="delete" src="resources/img/close.png"
									onclick="fn_cartDelete(this.form)" />
